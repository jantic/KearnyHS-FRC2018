...
public class ElevatorToPostionReset extends Command {
    ...
    public ElevatorToPostionReset(double targetDistance, double maxSpeed) {
    ...
    protected boolean isFinished() {
      if(Math.abs(distanceRemaining) < 30) { //may need to reduce this number as this was orginally used for encoders
        System.out.println("distance remiainigng " + this.distanceRemaining);
        elevatorSub.EncoderReset(this.currentDistance);
        return true;
      }
      else if(!lSwitch.get()) {
